  ...
  try {
    ...
    const translation = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "user",
          content: "Translate the following text to Spanish: 'Hello, how are you today?'"
        }
      ],
      max_tokens: 50,
    });
    ...
    const codeExplanation = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "user",
          content: "Explain what this JavaScript code does: const result = array.map(x => x * 2).filter(x => x > 10);"
        }
      ],
      max_tokens: 100,
    });

    console.log('Code Explanation:', codeExplanation.choices[0].message.content);
    console.log('');
